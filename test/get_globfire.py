...
def get_daily_fires(year, min_size=1e7, region=None):
    ...
    for day in days:
        try:
            ...
            polygons = ee.FeatureCollection(collection_name).filterBounds(region)

            polygons = ee.filter(ee.Filter.gte)
            ...
            polygons = polygons.filter(ee.Filter.gt("area", min_size)).filter(
                ee.Filter.lt("area", 1e20)
            )
...
def get_combined_fires(year, min_size=1e7, region=None):
    ...
    for fire_id in all_ids:
        ...
        daily_fire = (
            daily_gdf[daily_gdf["Id"] == fire_id] if not daily_gdf.empty else None
        )
        # Get final perimeter for this fire
        final_fire = (
            final_gdf[final_gdf["Id"] == fire_id] if not final_gdf.empty else None
        )
...
def analyze_fires(gdf):
    ...
    if "source" in gdf.columns:
        ...
        fires_with_both = (
            gdf.groupby("Id")["source"].nunique().where(lambda x: x > 1).count()
        )
