...
def test_reset():
    ...
    ), f"Exptected `config.log_dir` to be {DEFAULT_LOG_DIR}"
    assert (
        config.log_level == DEFAULT_LOG_LEVEL
    ), f"Expected `config.log_level` to be {DEFAULT_LOG_LEVEL}"

    assert (
        config.tiff_dir == DEFAULT_TIFF_DIR
    ), f"Expected `config.tiff_dir` to be {DEFAULT_TIFF_DIR}"
    assert (
        config.data_dir == DEFAULT_DATA_DIR
    ), f"Expected `config.data_dir` to be {DEFAULT_DATA_DIR}"
    assert (
        config.start_date == DEFAULT_START_DATE
    ), f"Expected `config.start_date` to be {DEFAULT_START_DATE}"
    assert (
        config.end_date == DEFAULT_END_DATE
    ), f"Expected `config.end_date` to be {DEFAULT_END_DATE}"
    assert (
        config.credentials == SERVICE_ACCOUNT
    ), f"Expected `config.credentials` to be {SERVICE_ACCOUNT}"
    assert (
        config.google_drive_dir == DEFAULT_GOOGLE_DRIVE_DIR
    ), f"Expected `config.google_drive_dir` to be {DEFAULT_GOOGLE_DRIVE_DIR}"
    assert (
        config.min_size == DEFAULT_MIN_SIZE
    ), f"Expected `config.min_size` to be {DEFAULT_MIN_SIZE}"
    assert (
        config.max_size == DEFAULT_MAX_SIZE
    ), f"Expected `config.max_size` to be {DEFAULT_MAX_SIZE}"
...
def test_dict_style_arguments():
    ...
    assert config.log_dir == LOG_DIR, f"Exptected `config.log_dir` to be {LOG_DIR}"
    assert config.log_level == LOG_LVL, f"Expected `config.log_level` to be {LOG_LVL}"
    ...
    assert config.start_date == datetime(
        2021, 1, 1
    ), "Expected `config.start_date` to be 2021-01-01"
    assert config.end_date == datetime(
        2021, 1, 14
    ), "Expected `config.end_date` to be 2021-01-14"
    assert (
        config.credentials == SERVICE_ACCOUNT
    ), f"Expected `config.credentials` to be {SERVICE_ACCOUNT}"
    assert (
        config.google_drive_dir == DRIVE_DIR
    ), f"Expected `config.google_drive_dir` to be {DRIVE_DIR}"
