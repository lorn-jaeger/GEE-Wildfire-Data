...
class FirePred:
    ...
    def compute_daily_features(
        self, start_time: str, end_time: str, geometry: Geometry
    ) -> ImageCollection:
        ...
        energy_release_component = (
            weather.select("erc").median().rename("energy release component")
        )
        ...
        weather_forecast = (
            self.weather_forecast.filter(
                Filter.gte("system:index", today_string + "00F01")
            )
            .filter(Filter.lte("system:index", today_string + "00F24"))
            .filterBounds(geometry)
        )
        forecast_temperature = (
            weather_forecast.select("temperature_2m_above_ground")
            .mean()
            .rename("forecast temperature")
        )
        forecast_specific_humidity = (
            weather_forecast.select("specific_humidity_2m_above_ground")
            .mean()
            .rename("forecast specific humidity")
        )
        forecast_u_wind = weather_forecast.select(
            "u_component_of_wind_10m_above_ground"
        ).mean()
        forecast_v_wind = weather_forecast.select(
            "v_component_of_wind_10m_above_ground"
        ).mean()

        # Transform from u/v to direction and speed, to align with GRIDMET and DEM data
        forecast_wind_speed = (
            forecast_u_wind.multiply(forecast_u_wind)
            .add(forecast_v_wind.multiply(forecast_v_wind))
            .sqrt()
            .rename("forecast wind speed")
        )
        forecast_wind_direction = forecast_v_wind.divide(forecast_u_wind).atan()
        forecast_wind_direction = (
            forecast_wind_direction.divide(2 * math.pi)
            .multiply(360)
            .rename("forecast wind direction")
        )

        # Rain forecasts were changed: From rain within the one-hour interval to cumulative rain during the day so far
        forecast_rain_change_date = datetime.datetime.strptime(
            "2019-11-07T06:00:00", "%Y-%m-%dT%H:%M:%S"
        )
        ...
        drought_index = (
            self.drought.filter(Filter.lte("system:time_start", today_timestamp))
            .filter(Filter.gte("system:time_end", today_timestamp))
            .select("pdsi")
            .median()
        )
        igbp_land_cover = (
            self.landcover.filterDate(
                start_time[:4] + "-01-01", start_time[:4] + "-12-31"
            )
            .filterBounds(geometry)
            .select("LC_Type1")
            .median()
        ...
        viirs_img = (
            self.viirs.filterDate(start_time, end_time)
            .filterBounds(geometry)
            .select(["M11", "I2", "I1"])
            .median()
        )
        viirs_veg_idc = (
            self.viirs_veg_idx.filterDate(
                (
                    datetime.datetime.strptime(end_time[:-6], "%Y-%m-%d")
