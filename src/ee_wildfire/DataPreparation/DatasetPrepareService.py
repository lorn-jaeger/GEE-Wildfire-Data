...
from ee import Geometry, ImageCollection, batch, data  # type: ignore
...
class DatasetPrepareService:
    ...
    def __init__(self, location: str, config: dict, user_config: UserConfig) -> None:
        ...
        self.geometry = Geometry.Rectangle(
            [
                self.longitude - self.rectangular_size,
                self.latitude - self.rectangular_size,
                self.longitude + self.rectangular_size,
                self.latitude + self.rectangular_size,
            ]
        )
    ...
    def prepare_daily_image(
        self,
        date_of_interest: str,
        time_stamp_start: str = "00:00",
        time_stamp_end: str = "23:59",
    ) -> ImageCollection:
        """
    ...
    def download_image_to_drive(
        self, image_collection: ImageCollection, index: str, utm_zone: str
    ) -> None:
        """
    ...
    def extract_dataset_from_gee_to_drive(
        self, utm_zone: str, n_buffer_days: int = 0
    ) -> None:
        """
        ...
        for i in range(time_dif.days):
            date_of_interest = str(
                self.start_time - buffer_days + datetime.timedelta(days=i)
            )
            ConsoleUI.print(f"Processing date: {date_of_interest}")

            try:
                img_collection = self.prepare_daily_image(
                ...
                if n_images > 1:
                    ...
                    ConsoleUI.error(
                        f"Found {n_images} features in img_collection returned by prepare_daily_image. "
                ...
                if len(max_img.getInfo().get("bands")) != 0:
                    self.download_image_to_drive(
                        img_collection, date_of_interest, utm_zone
                    )
