...
class DriveDownloader:
    ...
    def download_folder(self):
        ...
        try:
            ...
            ConsoleUI.add_bar(
                key="download", total=len(files), desc="Downloading files"
            )
            for file in files:
    ...
    def download_files(self):
        ...
        qm.wait_for_exports()
        expected_files = qm.get_task_filename()
        ...
        for fname in expected_files:
            ...
            try:
                ...
                if e.resp.status == 404:
                    ConsoleUI.print(
                        f"File {fname} not found (404). Skipping...", color="red"
                    )
                else:
                    ConsoleUI.print(
                        f"Unexpected error downloaded {fname}: {e}", color="red"
                    )
        ...
        try:
            while True:
                ...
                ConsoleUI.add_bar(
                    key="purge",
                    total=len(files),
                    desc="Purge progress (for first 1000 items)",
                    color="yellow",
                )
                ConsoleUI.print(f"found {len(files)} files")
