...
COMMAND_ARGS = {
    ...
    "--credentials": (
        ...
        DEFAULT_OAUTH_DIR,
        StorePassedAction,
        "Path to Google Authetication .json",
    ),
    "--data-dir": (
        Path,
        DEFAULT_DATA_DIR,
    ...
    "--tiff-dir": (
        ...
        DEFAULT_TIFF_DIR,
        StorePassedAction,
        "Path where downloaded tiff files go.",
    ),
    "--gdf-dir": (
        Path,
        DEFAULT_GDF_DIR,
        StorePassedAction,
        "Directory to store geodataframes",
    ),
    "--google-drive-dir": (
        str,
        DEFAULT_GOOGLE_DRIVE_DIR,
        StorePassedAction,
        "Google Drive folder for exporting.",
    ),
    "--min-size": (
        float,
        DEFAULT_MIN_SIZE,
        StorePassedAction,
        "Minimum size of fire area.",
    ),
    "--max-size": (
        float,
        DEFAULT_MAX_SIZE,
        StorePassedAction,
        "Maximum size of fire area.",
    ),
    ...
    "--purge-before": (
        ...
        False,
        "store_true",
        "Purge data from google drive before exporting",
    ),
    "--purge-after": (
        None,
        False,
        "store_true",
        "Purge data from google drive after downloading",
    ),
    "--start-date": (
        parse_datetime,
        DEFAULT_START_DATE,
        StorePassedAction,
        "Starting date for Earth Engine querry",
    ),
    "--end-date": (
        parse_datetime,
        DEFAULT_END_DATE,
        StorePassedAction,
        "Ending date for Earth Engine querry",
    ),
