...
def get_completed_tasks_versus_list(expected_files: List[str]):
    ...
    filtered_completed_tasks = [
        t for t in completed_tasks if t["description"] in fixed_files
    ]
    output = []
...
def process_locations(
    locations: List[str], user_config: UserConfig, fire_config: Dict
) -> List[str]:
    failed_locations = []
    ...
    for location in locations:
        ...
        dataset_pre = DatasetPrepareService(
            location=location, config=fire_config, user_config=user_config
        )
...
def export_data(yaml_path: Union[Path, str], user_config: UserConfig) -> bool:
    ...
    ConsoleUI.add_bar(
        key="failed",
        total=len(locations),
        desc="Number of failed locations",
        color="red",
    )
...
def get_location_count(yaml_path: Union[Path, str]):
    fire_config = load_fire_config(yaml_path)
    fire_names = list(fire_config.keys())
    for non_fire_key in ["output_bucket", "rectangular_size", "year"]:
        fire_names.remove(non_fire_key)
    return len(fire_names)
